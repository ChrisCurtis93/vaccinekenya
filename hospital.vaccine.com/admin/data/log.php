...
if (isset($_GET['email']) || isset($_GET['password'])){
	...
	$row = $db->GetRow("select * from tb_users_hospitals where hospital_email = '$email' and 
		hospital_password = '". md5(sha1($password)) . "'");
	if($row){
		if($row ["active_status"] == '1'){
			$myresults = array();
			$myresults["UserID"] = $row ["hospital_id"];
			$myresults["success"] = 1;
			$myresults["message"] = $row ["hospital_id"];
